#!/usr/bin/env bash
# test_macos.sh — End-to-end test on macOS (ch07+)
# Compiles each valid .mc file, assembles with clang -arch x86_64, runs, checks exit code.
set -euo pipefail
...
while IFS= read -r line; do
    ...
    # Compile
    if ! "$MINICC" -platform macos "$src" -o "$asm" 2>/dev/null; then
    ...
    # Assemble + link
    if ! clang "$asm" "$RUNTIME" -o "$bin" -arch x86_64 2>/dev/null; then
